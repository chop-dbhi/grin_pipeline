...
igvsnapshot<-function(df){
    ...
    #print(df[['chrom']])
    IGVgoto(sock, paste(df[['chrom']],':',df[['start']],'-',df[['end']],sep=""))
    #IGVsnapshot(sock, dirname=getwd())
    #fname = paste(familyname,varid,sep="")
}
#denovo %>% dplyr::rowwise() %>% dplyr::do(igvsnapshot(data.frame(chr=.$chrom,start=.$start,end=.$end)))
apply(denovo[1,],1,igvsnapshot)
#,variant_id)
