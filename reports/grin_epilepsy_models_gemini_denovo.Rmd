...
```{r denovo_igv, warnings = FALSE, echo=FALSE, message=FALSE, cache=FALSE,fig.path='{{SLINK}}/'}
    #, warnings = FALSE, eval=showChunk, echo=FALSE, message=FALSE, cache=FALSE,fig.path='{{SLINK}}/'}
sock <- IGVsocket()
IGVclear(sock)
#print(paste(projdir,bamouts,sep="/"))
IGVload(sock, paste(projdir,bamouts,sep="/"))
igvsnapshot<-function(df){
    #str(df) #[['chr']],':',df[['start']],'-',df[['end']],sep="")
    #print(df[['chrom']])
    goto<-str_replace_all(paste(df[['chrom']],':',df[['start']],'-',df[['end']],sep=""),' ','')
    IGVgoto(sock, goto)
    fname<-str_replace_all(paste(familyname,df[['variant_id']],'png',sep="."),' ','')
    IGVsnapshot(sock, dirname=paste(projdir,dirpath,sep="/"),fname=fname)
    #fname = paste(familyname,varid,sep="")
}
apply(denovo,1,igvsnapshot)
#http://mybic.chop.edu/slink/helbig/grinpros/GRCh37/analysis/%7B%7BSLINK%7D%7D/GRCh37/analysis/EG0011_EG0011P.1641.png
denovo$variant_id<-paste("<a href=\"",paste(paste(familyname,denovo$variant_id,'png',sep="."),sep="/"),"\">",denovo$variant_id,"</a>",sep="")
#dirpath,
#,variant_id)
```
