...
denovo.sub <- denovo.sub[which(denovo.sub$exac_ac_all == "None" | denovo.sub$exac_ac_all == "0"),]
...
1. Allelic ratio between 20%-80%.
2. 0 DP in mom and dad.
3. Genotyping quality filter: GQ > 20.
4. Only chr 1-22, X, Y, MT.

```{r denovo_filtered_enhancement, eval=showChunk, warning = FALSE, echo=FALSE, message=FALSE, cache=FALSE,  fig.path='{{SLINK}}/'}
denovo.sub <- denovo
chr <- c(1:22,'X','Y','MT')
denovo.sub <- denovo.sub[which(denovo.sub$chr %in% chr),]
denovo.sub$chr <- factor(denovo.sub$chr, levels = chr)
denovo.sub <- denovo.sub[order(denovo.sub$chr),]

denovo.sub$exac_ac_all[denovo.sub$exac_ac_all=="None"] <- "-1"
denovo.sub$exac_ac_all <- as.numeric(denovo.sub$exac_ac_all)

quals <- colsplit(gsub("\\s+"," ",stringr::str_trim(gsub('[[]|[]]|[.]',' ',denovo.sub$gt_quals)))," ",c('mom','dad','kid'))
denovo.sub <- denovo.sub[which(apply(quals, MARGIN = 1, FUN = function(x) all(x>20))),]
