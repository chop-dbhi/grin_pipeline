...
if (nrow(in_recurrent) == 0){
  ...
  varcount <- unique(length(in_recurrent$variant_id))
}
```
...
if (nrow(in_literature) == 0){
  ...
  viewDataTable(dat = in_literature)
    varcount <- unique(length(in_literature$variant_id))
}
```
...
if (nrow(codingonly) == 0){
  ...
  varcount <- unique(length(codingonly$variant_id))
}
```
...
if (nrow(others) == 0){
  ...
  varcount <- unique(length(others$variant_id))
}
```
...
```{r arhomo_coding, eval=TRUE, warning = FALSE, echo=FALSE, message=FALSE, cache=FALSE,  fig.path='{{SLINK}}/'}
