...
PDFS = [config['process_dir'][freeze] + config['results']['pdfs'] + "/" + name + ".pdf" for name in EXISTINGSAMPLES]
...
rule recalibrate_bam:
    ...
    output:
        ...
        bai = temp(config['process_dir'][freeze] + config['results']['recalibrated'] + "/{sample}.bai")
    log:
...
rule make_index:  # create bai files if needed
    input:
...
rule add_readgroup:
    input:
        ...
        java = ENV3 + config['tools']['java']
    output:
        bam = config['process_dir'][freeze] + config['results']['picard'] + "/{sample}.group.bam", # needed by run_multiqc
...
rule make_gvcf:
    ...
    output:
        ...
        gidx = temp(config['process_dir'][freeze] + config['results']['gvcfs'] + "/{sample}.gvcf.idx")
    params:
        jar = config['jars']['gatk'],
        opts = config['tools']['opts']['med'] + ' ' + config['javatmpdir'],
        ref = config['ref'][freeze]
...
def gvcf_samples_in_family(family,subject):
    ...
    if family == 'joint':
        ...
        gvcfs = [config['process_dir'][freeze] + config['results']['gvcfs'] + "/" + name + ".gvcf" for name in set(samples)]
        gvcfslist = ' '.join(["--variant " + config['process_dir'][freeze] + config['results']['gvcfs'] + "/" + name + ".gvcf" for name in set(samples)])
        return [gvcfs, gvcfslist]
